...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 16 10:27:59 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue May 24 12:44:51 2022
...
for excludeSubhalos in excludeSubhaloss: # completed running for [3]
    if excludeSubhalos:
    ...
    for redbini in np.arange(snap_z_info.shape[0]):
        ...
        selParInds=np.arange(len(Xnames))
        selStrs=['hi','lo']
        
        for selParInd in selParInds:
            for selStr in selStrs:
                
                print('==================== (current parameter, selection): (%s, %s)' %(Xnames[selParInd],selStr))
                selPar=Xnames[selParInd]
                ...
                #     for j in range(len(Xnames)):
                        if j>i:
                            ...
                            # load data
                            # ticload=time.time()
                            fileName = 'selHaloParBy_%s_%s_z0_profiles_%s_%s%s_snapshot%d.npz' %(selPar,selStr,proxyname1,proxyname2,subhaloStr,snapshot)
                            profData=np.load(loadDir+fileName)
                            ...
                            spinMeanInBin=profData['spinMeanInBin']
                            eMeanInBin=profData['eMeanInBin']
                            delta_eMeanInBin=profData['delta_eMeanInBin'] 
                            lim_z0=profData['lim_z0']
                            
                            
                            # ##### convert delta_e to np.log(1+delta_e)!!!
                            # delta_eMeanInBin=np.log10(1+delta_eMeanInBin)#delta_e===np.log(1+delta_e)
                            # if j==5:
                            #     parBin2=np.log10(1+parBin2)
                            #     haloParMean[:,1]=np.log10(1+haloParMean[:,1])
                            #     haloParStd[:,1]=np.log10(1+haloParStd[:,1])
                            
                            #if above doesn't work try below:
                            # check shape of rho first
                            # rho_e=np.mean(rho[:,(r>=1)&(r<=2)],axis=1)
                            # delta_eMeanInBin=rho_e/rhom-1
                            # delta_eMeanInBin=np.log10(1+delta_eMeanInBin)
                            ...
                            j2d=make2d(spinMeanInBin)
                            e2d=make2d(eMeanInBin)
                            deltae2d=make2d(delta_eMeanInBin) 
                            ...
                            fileName='selHaloParBy_%s_%s_z0_z_maps_HaloPar%d%d_nbin%d%s_snapshot%d' %(selPar,selStr,i, j, nbin,subhaloStr,snapshot)
                            toc=time.time()
                            print(fileName+': %.2f s' %(toc-tic))
                            np.savez(saveDir+fileName, 
                                     parBin1=parBin1,parBin2=parBin2,
                                     X1=X2d.ravel(),X2=Y2d.ravel(),
                                     ...
                                     js=j2d.ravel(),es=e2d.ravel(),deltaes=deltae2d.ravel(),
                                     lim_z0=lim_z0,
                                     )#delta_e===np.log(1+delta_e)
